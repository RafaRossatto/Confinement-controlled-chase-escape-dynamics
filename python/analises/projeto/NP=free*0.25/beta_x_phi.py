...
OUT_DIR = BASE_ROOT /"L_128"/"resultados_modelos" /"paper_response"/ "beta_x_phi"
...
for base_label, base_nome in bases:
    ...
    for obstaculo in OBSTACULOS:
        ...
        if arquivo.exists():
            ...
            if 'beta' in df.columns and 'tau_err' in df.columns:
                ...
                resultados[base_nome][ob_num] = {
                    ...
                    'estatisticas': {
                        ...
                        'media_beta_err': df['beta_err'].mean()
                    }
                }
...
for idx, (label_tex, nc_tag) in enumerate(bases):
    ...
    for obstaculo in OBSTACULOS:
        ...
        if 'beta' not in df.columns or 'beta_err' not in df.columns:
            print(f"  Colunas beta/beta_err não encontradas em: {fpath.name}")
            continue
        ...
        beta_err_mean = df['beta_err'].mean()
        
        phis.append(phi)
        taus.append(beta_mean)
        tau_errs.append(beta_err_mean)

        print(f"  Obstáculo {ob_num}: phi={phi:.4f}, beta={beta_mean:.4f}")
