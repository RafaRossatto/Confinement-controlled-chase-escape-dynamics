...
int SEARCHRADIUS = 5; // raio de procura.
...
const std::vector<Obstacle>& obstaculos,int teste) 
{
    ...
    // finaliza a verificão se o ponto está ocupado por outra célula de proteção.
...
/**
 * @brief Checks for overlap of a point with obstacles, resources, or cells.
 *
 * This function checks if the point specified by ponto overlaps with any point in
 * the pontos vector, any resource in the recursos vector, or any cell in the 
 * celulas vector based on their coordinates.
 *
 * @param ponto The point to check for overlap.
 * @param pontos Vector of obstacles (points) to check against.
 * @param recursos Vector of resources to check against.
 * @param celulas Vector of cells to check against.
 * @return true if the point overlaps with any obstacle, resource, or cell; false otherwise.
 */

bool verificaSobreposicao(const Obstacle& ponto, const std::vector<Obstacle>& pontos, 
    const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas) 
{
    for (const auto& outra_ponto : pontos) 
    {
        if (ponto.getCoordenadaX() == outra_ponto.getCoordenadaX() && ponto.getCoordenadaY() == outra_ponto.getCoordenadaY()) {
            return true; // Point overlaps with another point
        }
    }

    for (const auto& recurso : recursos) {
        if (ponto.getCoordenadaX() == recurso.getCoordenadaX() && ponto.getCoordenadaY() == recurso.getCoordenadaY()) {
            return true; // Point overlaps with another point
        }
    }

    for (const auto& celula : celulas) {
        if (ponto.getCoordenadaX() == celula.getCoordenadaX() && ponto.getCoordenadaY() == celula.getCoordenadaY()) {
            return true; // Point overlaps with another point
        }
    }
...
/**
 * @brief Checks for overlap of a resource with obstacles, other resources, or cells.
 *
 * This function checks if the resource specified by recurso overlaps with any obstacle
 * in the pontos vector, any other resource in the recursos vector, or any cell in the
 * celulas vector based on their coordinates.
 *
 * @param recurso The resource to check for overlap.
 * @param pontos Vector of obstacles (points) to check against.
 * @param recursos Vector of other resources to check against.
 * @param celulas Vector of cells to check against.
 * @return true if the resource overlaps with any obstacle, other resource, or cell; false otherwise.
 */

bool verificaSobreposicao(const Recurso& recurso, const std::vector<Obstacle>& pontos, 
const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas) 
{
    for (const auto& ponto : pontos) 
    {
        if (recurso.getCoordenadaX() == ponto.getCoordenadaX() && recurso.getCoordenadaY() == ponto.getCoordenadaY()) {
            return true; // Resource overlaps with a point
        }
    }

    for (const auto& outro_recurso : recursos) {
        if (recurso.getCoordenadaX() == outro_recurso.getCoordenadaX() && recurso.getCoordenadaY() == outro_recurso.getCoordenadaY()) {
            return true; // Resource overlaps with another resource
        }
    }

    for (const auto& celula : celulas) {
        if (recurso.getCoordenadaX() == celula.getCoordenadaX() 
        && recurso.getCoordenadaY() == celula.getCoordenadaY()) 
        {
            return true; // Resource overlaps with a cell
        }
    }
...
/**
 * @brief Checks for overlap of a cell with obstacles, resources, or other cells.
 *
 * This function checks if the cell specified by celula overlaps with any obstacle
 * in the pontos vector, any resource in the recursos vector, or any other cell in the
 * celulas vector based on their coordinates.
 *
 * @param celula The cell to check for overlap.
 * @param pontos Vector of obstacles (points) to check against.
 * @param recursos Vector of resources to check against.
 * @param celulas Vector of other cells to check against.
 * @return true if the cell overlaps with any obstacle, resource, or other cell; false otherwise.
 */

bool verificaSobreposicao(const Cell& celula, const std::vector<Obstacle>& pontos, 
const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas) 
{
    for (const auto& ponto : pontos) {
        if (celula.getCoordenadaX() == ponto.getCoordenadaX() && celula.getCoordenadaY() == ponto.getCoordenadaY()) 
        {
            return true; // Cell overlaps with a point
        }
    }

    for (const auto& recurso : recursos) 
    {
        if (celula.getCoordenadaX() == recurso.getCoordenadaX() && celula.getCoordenadaY() == recurso.getCoordenadaY()) 
        {
            return true; // Cell overlaps with a resource
        }
    }

    for (const auto& outra_celula : celulas) 
    {
        if (celula.getCoordenadaX() == outra_celula.getCoordenadaX() && celula.getCoordenadaY() == outra_celula.getCoordenadaY()) {
            return true; // Cell overlaps with another cell
        }
    }
...
/**
 * @brief Checks for overlap of a cancer cell with obstacles, resources, other cells, or other cancer cells.
 *
 * This function checks if the cancer cell specified by cancer overlaps with any obstacle
 * in the pontos vector, any resource in the recursos vector, any cell in the celulas vector,
 * or any other cancer cell in the cancers vector based on their coordinates.
 *
 * @param cancer The cancer cell to check for overlap.
 * @param pontos Vector of obstacles (points) to check against.
 * @param recursos Vector of resources to check against.
 * @param celulas Vector of cells to check against.
 * @param cancers Vector of other cancer cells to check against.
 * @return true if the cancer cell overlaps with any obstacle, resource, cell, or other cancer cell; false otherwise.
 */
bool verificaSobreposicao(const Cell& cancer, const std::vector<Obstacle>& pontos, 
const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas, 
const std::vector<Cell>& cancers) 
{
    for (const auto& ponto : pontos) {
        if (cancer.getCoordenadaX() == ponto.getCoordenadaX() && cancer.getCoordenadaY() == ponto.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a point
        }
    }

    for (const auto& recurso : recursos) {
        if (cancer.getCoordenadaX() == recurso.getCoordenadaX() && cancer.getCoordenadaY() == recurso.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a resource
        }
    }

    for (const auto& celula : celulas) {
        if (cancer.getCoordenadaX() == celula.getCoordenadaX() && cancer.getCoordenadaY() == celula.getCoordenadaY()) {
            return true; // Cancer cell overlaps with a cell
        }
    }

    for (const auto& outra_cancer : cancers) {
        if (cancer.getCoordenadaX() == outra_cancer.getCoordenadaX() && cancer.getCoordenadaY() == outra_cancer.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with another cancer cell
        }
    }
 ...
 */
 ...
 bool posicionarObjetos(std::vector<Obstacle>& pontos, std::vector<Recurso>& recursos, 
    ...
    int num_celulas, int num_cancers, int largura_max, int altura_max,std::mt19937& rng) 
    {
        ...
        // ⚠️ Se obstáculos já foram carregados, não adicionar mais
        if (num_pontos != 0) {
            std::cout << "[INFO] Obstáculos já carregados: " << pontos.size() << std::endl;
            ...
            while (tentativas_pontos < max_tentativas && pontos.size() < num_pontos) {
                ...
                if (!verificaSobreposicao(novo_ponto, pontos, recursos, celulas)) {
        ...
        while (tentativas_recursos < max_tentativas && recursos.size() < num_recursos) {
            ...
            if (!verificaSobreposicao(novo_recurso, pontos, recursos, celulas)) {
        ...
        while (tentativas_celulas < max_tentativas && celulas.size() < num_celulas) {
            ...
            if (!verificaSobreposicao(nova_celula, pontos, recursos, celulas)) {
        ...
        while (tentativas_cancers < max_tentativas && cancers.size() < num_cancers) {
            ...
            if (!verificaSobreposicao(nova_cancer, pontos, recursos, celulas, cancers)) {
...
const std::vector<Cell>& cancers) 
{
    ...
    for (const auto& o_recurso : recursos) {
        if (recurso.getCoordenadaX() == o_recurso.getCoordenadaX() && recurso.getCoordenadaY() == o_recurso.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a resource
        }
    }
    ...
    for (const auto& celula : celulas) {
        if (recurso.getCoordenadaX() == celula.getCoordenadaX() && recurso.getCoordenadaY() == celula.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a cell
        }
    }
    ...
    for (const auto& cancer : cancers) {
        if (recurso.getCoordenadaX() == cancer.getCoordenadaX() && recurso.getCoordenadaY() == cancer.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with another cancer cell
        }
    }
...
// verifica a sobreposição da célula de cancer.

bool verificaSobreposicao1(const Cell& cancer, const std::vector<Obstacle>& pontos, 
const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas, 
const std::vector<Cell>& cancers) 
{
    for (const auto& ponto : pontos) {
    ...
    for (const auto& o_cancer : cancers) {
        if (cancer.getCoordenadaX() == o_cancer.getCoordenadaX() && cancer.getCoordenadaY() == o_cancer.getCoordenadaY()) {
            return true; // Cancer cell overlaps with a resource
        }
    }
...
bool verificaSobreposicao2(const Cell& celula, const std::vector<Obstacle>& pontos, 
const std::vector<Recurso>& recursos, const std::vector<Cell>& celulas, 
const std::vector<Cell>& cancers) 
{
    for (const auto& ponto : pontos) {
    ...
    for (const auto& o_cancer : cancers) {
        if (celula.getCoordenadaX() == o_cancer.getCoordenadaX() && celula.getCoordenadaY() == o_cancer.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a resource
        }
    }

    for (const auto& o_celula : celulas) {
        if (celula.getCoordenadaX() == o_celula.getCoordenadaX() && celula.getCoordenadaY() == o_celula.getCoordenadaY()) 
        {
            return true; // Cancer cell overlaps with a cell
        }
    }
...
/*
bool posicionarTodosRecursos(std::vector<Recurso>& recursos, const std::vector<Obstacle>& pontos,
const std::vector<Cell>& celula,const std::vector<Cell>& cancer,const int nRecursos)
{
    //srand(time(0)); // Inicializa a semente de números aleatórios uma vez
    int max_tentativas = 100;

    int recursos_iniciais = recursos.size(); // Número de recursos já no vetor
    int total_recursos = recursos_iniciais + nRecursos; // Total desejado após adição
    ...
    for (int i = recursos_iniciais; i < total_recursos; ++i) 
    {
        ...
        for (int tentativa = 0; tentativa < max_tentativas && !posicionado; ++tentativa) 
        {
            ...
            int x = distribX(global_gen);

        
            std::uniform_int_distribution<int> distribY(0, HEIGHT - 1);
            int y = distribY(global_gen);
            Recurso novo_recurso = {"P", i+1, x, y, ENERGY}; // Assume ENERGY é um valor definido

            if (!verificaSobreposicao(novo_recurso, pontos, recursos, celula, cancer)) 
            {
                recursos.push_back(novo_recurso);
                posicionado = true;
            }
...
int cancerEnergy, int nCancer,std::mt19937& rng)
{
    ...
    for (int i = recursos_iniciais; i < total_recursos; ++i) 
    {
        ...
        for (int tentativa = 0; tentativa < max_tentativas && !posicionado; ++tentativa) 
        {
            ...
            if (!verificaSobreposicao1(nova_celula, pontos, Recurso, celula, cancer)) 
            {
...
const  int cellEnergy,const int nCell,std::mt19937& rng)
{
    ...
    for (int i = recursos_iniciais; i < total_recursos; ++i) 
    {
        ...
        for (int tentativa = 0; tentativa < max_tentativas && !posicionado; ++tentativa) 
        {
            ...
            if (!verificaSobreposicao2(nova_celula, pontos, Recurso, celula, cancer)) 
            {
...
double calculateDistance(int x1, int y1, int x2, int y2) 
{
    ...
    // Considera o menor caminho levando em conta as condições periódicas
...
std::vector<Recurso>& food, const std::string& fileName, std::mt19937& rng, bool write = false) 
{
    ...
    {   
        ...
        std::cout << "Vai escrever. " <<  write << std::endl;
            //write = false;    
            //std:: cin.get();
        }
        ...
        if (cC.empty()) 
        {
            std::cout << "No more cancer cells. Terminating program." << std::endl;
            break;
        }
...
int main() {
    ...
    int SR_value = 50;
    unsigned int seed_run = 3077551527; // do .dat
    ...
    if (!posicionarObjetos(point, recursos, cT, cC, numPoint, numRecursos, numCT,
         numcC, 100, 100,rng_local)) 
    {
        std::cerr << "Erro ao posicionar objetos para run " << run << std::endl;
        return 1;
    }
    ...
    int steps_local = runSimulationPaper(10000, cT, cC, point, recursos, trajectoryFileName,rng_local, true);

    std::cout << "Re-execução completa com " << steps_local << " passos." << std::endl;
